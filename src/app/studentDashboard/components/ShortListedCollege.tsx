...
const MOCK_COLLEGES = [
  {
    ...
    affiliated_to: "University of Delhi",
    application_status: "not_applied",
    saved_on: "2025-09-17",
    student_note: "Strong academics and city opportunities",
    image: "/college1.jpg",
    fees: "₹12,000/yr",
  ...
  },
  {
    ...
    affiliated_to: "University of Madras",
    application_status: "not_applied",
    saved_on: "2025-09-15",
  ...
  },
  {
    ...
    application_status: "not_applied",
    saved_on: "2025-09-14",
    student_note: "Strong research culture",
    image: "/college4.jpg",
    fees: "₹20,000/yr",
...
const FILTERS = {
  district: ["All", "South Delhi", "Mumbai City", "Chennai", "Varanasi", "Bengaluru Urban"],
...
export default function ShortListedCollege() {
  ...
  const counselorComments: { [id: string]: string } = {
    "IN-GOV-001": "Recommended for commerce and humanities streams.",
    "IN-PRI-002": "Check hostel and fee structure before final submission.",
    "IN-GOV-003": "Excellent labs and senior faculty support.",
    "IN-CEN-004": "Great for long-term research-focused goals.",
    "IN-PRI-005": "Strong management and industry exposure.",
  };
...
//       setLoading(true);
//       const res = await fetch("/api/user/shortlistedColleges");
//       const data = await res.json();
//       if (data.success) {
//         setColleges(data.data);
//         setNotes(Object.fromEntries(data.data.map((c: any) => [c.college_id, c.student_note || ""])))
//       }
//       setLoading(false);
//     }
//     fetchColleges();
...
//   // Add, remove, update note, update status handlers
//   async function handleRemove(college_id: string) {
//     await fetch("/api/user/shortlistedColleges", {
//       method: "DELETE",
//       headers: { "Content-Type": "application/json" },
//       body: JSON.stringify({ college_id }),
//     });
//     setColleges(colleges.filter(c => c.college_id !== college_id));
//   }

//   async function handleNoteChange(college_id: string, note: string) {
//     setNotes(n => ({ ...n, [college_id]: note }));
//     await fetch("/api/user/shortlistedColleges", {
//       method: "PATCH",
//       headers: { "Content-Type": "application/json" },
//       body: JSON.stringify({ college_id, student_note: note }),
//     });
//   }

//   async function handleApply(college_id: string) {
//     await fetch("/api/user/shortlistedColleges", {
//       method: "PATCH",
//       headers: { "Content-Type": "application/json" },
//       body: JSON.stringify({ college_id, application_status: "applied" }),
//     });
//     setColleges(colleges.map(c => c.college_id === college_id ? { ...c, application_status: "applied" } : c));
//   }
...
//   // Loading state
//   if (loading) {
//     return <div className="flex items-center justify-center min-h-[60vh] text-xl text-indigo-600">Loading your shortlisted colleges...</div>;
//   }
...
//                 placeholder="Add a note..."
//                 value={notes[college.college_id] || ""}
//                 onChange={e => handleNoteChange(college.college_id, e.target.value)}
//               />
//               {/* Actions */}
//               <div className="flex gap-2 mt-2 items-center">
//                 <button className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-2 py-1 rounded-lg shadow transition-all">View Details</button>
//                 {college.application_status !== "applied" && (
//                   <button className="bg-green-600 hover:bg-green-700 text-white font-bold px-2 py-1 rounded-lg shadow transition-all" onClick={() => handleApply(college.college_id)}>Apply Now</button>
//                 )}
//                 {college.application_status === "applied" && (
//                   <span className="text-green-700 font-semibold px-2 py-1">Applied on {college.saved_on}</span>
//                 )}
